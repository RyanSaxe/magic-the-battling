...
export function PlayerList({ players, currentPlayerName, currentPlayer, useUpgrades }: PlayerListProps) {
  ...
  const opponents = showOthersTab
    ? nonSelfPlayers.filter((p) => p.pairing_probability !== null && p.pairing_probability > 0)
    : nonSelfPlayers
        ...
        <div className="space-y-3">
          <div className="relative flex items-center justify-center gap-6">
            ...
            <div className="flex flex-col items-center gap-1">
              <img src={POISON_COUNTER_IMAGE} alt="Poison" className="h-16 rounded" />
              <span className="text-lg font-bold text-purple-400">{currentPlayer.poison}</span>
            </div>
            <div className="flex flex-col items-center gap-1">
              <img src={TREASURE_TOKEN_IMAGE} alt="Treasure" className="h-16 rounded" />
              <span className="text-lg font-bold text-amber-400">{currentPlayer.treasures}</span>
            </div>
          </div>
      ...
      {activeTab === 'opponents' && (
        <div className="space-y-2">
          ...
          {sortedOpponents.length === 0 && (
            <div className="text-gray-500 text-sm text-center py-4">No opponents</div>
          )}
        </div>
      )}
